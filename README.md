...
**メール認証誘導画面**
- 要件の一部に下記のように記載があるため、「認証はこちらから」ボタンを押下するとmailhog(http://localhost:8025/)サイトを別タグで表示するように設定しております。
1. メール認証導線画面を表示する
2. 「認証はこちらから」ボタンを押下
3. メール認証サイトを表示する
- mailhogにて直近に届いたメールを開き「Verify Email Address」ボタンを押下すると、プロフィール設定画面(初回)が別タグで開きます。
